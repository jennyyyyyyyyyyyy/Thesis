...
class TrainingCamera1 : AppCompatActivity() {
...
//        countdownOverlay = findViewById(thesis.filconnected.R.id.countdown_overlay) // Use XML TextView
//        countdownOverlay.visibility = View.GONE

//        countdownOverlay = TextView(this).apply {
//            textSize = 36f
...
//        }
//        addContentView(countdownOverlay, previewView.layoutParams)
    ...
    private fun askForLabel() {
        ...
        dialog.setCancelable(false) // Allow dismissing when clicking outside
    ...
    private fun startStaticGestureCollection() {
        ...
        btnNext.setOnClickListener {
            ...
            if (userLabel.isNotEmpty()) {
                ...
                alertDialog.dismiss() // Fix: Use correct dialog reference
            } else {
        ...
        val checkCompletionTask = object : Runnable {
            override fun run() {
                if (isDataCollectionComplete) {
                    ...
                    val proceedDialog = AlertDialog.Builder(this@TrainingCamera1)
                        .setTitle("Proceed to Next Key Pose?")
                        .setMessage("Do you want to capture the next key pose for '$userLabel'?")
                        .setPositiveButton("Yes") { _, _ ->
                            // Reset the flag for the next key pose
